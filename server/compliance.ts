...
2. 景品表示法（不当景品類及び不当表示防止法）
   ...
   - 打消し表示が不十分な場合
   - 「お客様の声」「口コミ」の捏造・誇張
   【キャンペーン・懸賞広告の規制（景品類の規制）】
   - 一般懸賞（抽選・クイズ等）：景品額が取引価額の20倍または10万円を超える場合は違反
   - 総付景品（来場・購入者全員プレゼント）：景品額が取引価額の10分の2（20%）を超える場合は違反
   - 共同懸賞（商店街・複数事業者）：景品額が取引価額の50倍または30万円を超える場合は違反
   - 「必ず当たる」「全員プレゼント」の虚偽表示（実際には抽選・条件付きの場合）
   - 当選確率・当選者数・応募総数の非表示または虚偽表示
   - 「先着〇名様」の虚偽（実際には先着ではなく抽選の場合）
   - キャンペーン応募条件（対象商品・期間・地域・年齢制限等）の不明示または曖昧な表示
   - 景品の提供条件（いつ・どのように受け取れるか）の不明示
   - SNS懸賞キャンペーン（フォロー＆リポスト等）における景品規制の適用（景品類の上限・条件明示）
   - オープン懸賞（購入不要）と クローズド懸賞（購入必要）の区別が不明確な表示
   - キャンペーン特典を「プレゼント」と称しつつ実質的な購入条件がある場合の不明示
   - 「ポイント〇倍」「〇%還元」などの特典条件（対象商品・上限・有効期限）の不明示
